# This code allows to change colours
# ===============================
# 1. Import Libraries
# ===============================
import pandas as pd
...
# ===============================
# 2. COLOR CONFIGURATION (EASILY ADJUSTABLE)
# ===============================
# Choose color scheme for K-means clusters (PCA plot)
PCA_PALETTE = "Set2"  # Options: "Set1", "Set2", "Set3", "husl", "pastel", "dark", "colorblind", etc.
# Or provide custom colors as a list:
# PCA_PALETTE = ["#FF6B6B", "#4ECDC4", "#45B7D1", "#FFA07A"]

# Choose color scheme for Dendrogram clusters (Heatmap)
# Options: 1) "auto" (automatic HSV), 2) list of custom colors, 3) specific palette name
DENDROGRAM_COLORS = "auto"  # or provide custom: ["#FF6B6B", "#4ECDC4", "#45B7D1", "#FFA07A", "#FFD93D"]

# Alternative pre-defined color schemes for dendrogram:
# DENDROGRAM_COLORS = "tab20"  # matplotlib colormap
# DENDROGRAM_COLORS = "husl"   # seaborn colormap

# ===============================
# 3. Load Data
# ===============================
file_path = "3_ABC Full Data Table_Top alleles_Harsh 16.csv"
...
# ===============================
# 7. K-means Clustering (Clusters start from 1)
# ===============================
...
# ===============================
# 8. PCA Plot (colored by cluster with customizable colors)
# ===============================
...
sns.scatterplot(
    ...
    y="PC2",
    hue="Cluster",
    palette=PCA_PALETTE,  # Uses color configuration from section 2
    s=80,
    legend='full'
)
...
# ===============================
# 10. Helper Function for Dendrogram Color Mapping
# ===============================
def get_dendrogram_colors(n_clusters, color_scheme):
    """
    Generate colors for dendrogram clusters.
    
    Parameters:
    -----------
    n_clusters : int
        Number of clusters to color
    color_scheme : str or list
        - "auto": Use HSV colormap (default)
        - "tab20", "husl", "pastel", etc.: Use matplotlib/seaborn colormap
        - List of hex colors or color names: Use provided colors
    
    Returns:
    --------
    dict : mapping of cluster number to RGBA color
    """
    if isinstance(color_scheme, list):
        # Custom color list provided
        colors = color_scheme
        if len(colors) < n_clusters:
            print(f"Warning: Provided {len(colors)} colors but need {n_clusters}. Cycling colors.")
            colors = (colors * (n_clusters // len(colors) + 1))[:n_clusters]
        # Convert to RGBA if needed
        color_list = []
        for color in colors:
            try:
                # Try to convert to matplotlib color
                import matplotlib.colors as mcolors
                rgba = mcolors.to_rgba(color)
                color_list.append(rgba)
            except:
                print(f"Warning: Could not parse color {color}")
                color_list.append((0.5, 0.5, 0.5, 1.0))  # Default gray
        return color_list
    
    elif color_scheme == "auto":
        # Use HSV colormap (original behavior)
        colors = plt.cm.hsv(np.linspace(0, 0.95, n_clusters))
        return colors
    
    else:
        # Try to use as matplotlib/seaborn colormap
        try:
            cmap = plt.get_cmap(color_scheme)
            colors = cmap(np.linspace(0, 1, n_clusters))
            return colors
        except:
            print(f"Warning: Colormap '{color_scheme}' not found. Using 'auto' (HSV).")
            colors = plt.cm.hsv(np.linspace(0, 0.95, n_clusters))
            return colors

# ===============================
# 11. Clustered Heatmap with Dendrogram (with color-coded groups)
# ===============================
...
# Create color mapping for clusters using configuration from section 2
unique_clusters = np.unique(dendro_clusters)
n_clusters = len(unique_clusters)

colors = get_dendrogram_colors(n_clusters, DENDROGRAM_COLORS)
...
# ===============================
# 12. Extract Dendrogram Groupings and Export
# ===============================
